...
internal class WriteTrnsActTest {
    ...
    @Test
    fun `Testing create a new transaction with expense`() = runBlocking<Unit> {
        val account = Account(
            id = UUID.randomUUID(),
            name = "Test account",
            currency = "EUR",
    ...
    @Test
    fun `Testing create a new transaction with income`() = runBlocking<Unit> {
        val account = Account(
            id = UUID.randomUUID(),
            name = "Test account",
            currency = "EUR",
        ...
        assertThat(cachedTransaction?.amount).isEqualTo(500.0)
        assertThat(cachedTransaction?.currency).isEqualTo("EUR")
        assertThat(cachedTransaction?.state).isEqualTo(TrnState.Default)
        assertThat(cachedTransaction?.purpose).isEqualTo(TrnPurpose.Fee)
